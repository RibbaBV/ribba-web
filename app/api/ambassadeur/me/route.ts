...
import { leesCampagne } from '@/lib/ribba-ambassadeur';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data: uitbetalingen } = await supabase
      ...
      .select('id, referral_id, amount_cents, status, geclaimd_op, uitbetaald_op, mislukking_reden, created_at')
      .eq('ambassadeur_id', amb.id)
      .order('created_at', { ascending: false })
      .limit(100);

    const rijen = (uitbetalingen ?? []) as Array<Pick<UitbetalingRow,
      'id' | 'referral_id' | 'amount_cents' | 'status' | 'geclaimd_op' | 'uitbetaald_op' | 'mislukking_reden' | 'created_at'>>;
    ...
    return NextResponse.json({
      ...
      ambassadeur: {
        ...
        verificatie_nodig: partner.payouts_enabled !== true && som(['te_innen', 'geclaimd']) > 0,
      },
      tips: (tips ?? []) as Array<Pick<TipRow,
        'id' | 'school_naam' | 'status' | 'beloning_cents' | 'aangemeld_op' | 'verdiend_op'>>,
      uitbetalingen: rijen,
      totalen: {
        te_innen_cents: som(['te_innen']),
